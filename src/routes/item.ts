...
itemRouter.post('/items', authenticateAdmin(), fileUpload({ useTempFiles: true, limits: { fileSize: 10 * 1024 * 1024 } }), createItemSchema, ItemController.createItem());
itemRouter.patch('/items/status/:id', authenticateAdmin(), toggleItemStatusSchema, ItemController.toggleItemStatus());
...
itemRouter.post('/parent-items', authenticateAdmin(), createParentItemSchema, ItemController.createParentItem());
itemRouter.get('/parent-items', ItemController.fetchAllParentItems());
itemRouter.post('/bundles', authenticateAdmin(), createBundleSchema, ItemController.createBundle());
